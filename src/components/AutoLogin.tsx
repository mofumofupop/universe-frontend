...
export default function AutoLogin() {
  ...
  useEffect(() => {
    ...
    console.log("AutoLogin: storage read", {
      userId,
      passwordHashMask: passwordHash ? `${passwordHash.slice(0, 6)}...` : null,
    });
    ...
    if (
      ...
      console.log(
        "AutoLogin: not on root path, skipping",
        window.location.pathname
      );
      setStatus("skipped");
      return;
    ...
    if (!isHashLike) {
      console.warn(
        "AutoLogin: stored passwordHash doesn't look like SHA-256 hex; clearing storage to avoid repeated failures",
        { userId, passwordHash }
      );
      try {
        clearAuthFromStorage();
    ...
    (async () => {
      console.log("AutoLogin: attempting to authenticate", {
        userId,
